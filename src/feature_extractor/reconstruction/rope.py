...
from feature_extractor.typing import Tensor, HEAD_DIM, SEQUENCE
...
class SimplifiedRoPEV1:
    ...
    def create_rope_matrix_full_sequence(
        ...
        relative_position_matrix = torch.arange(sequence_length).unsqueeze(0) - torch.arange(sequence_length).unsqueeze(1)
        ...
        return torch.stack([
            torch.stack(row, dim=0)
            for row in matrices
        ], dim=0).to(self.device).to(self.dtype)
