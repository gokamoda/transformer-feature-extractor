...
class StaticSDPAConfig:
    """Config whose attn_implementation is permanently 'sdpa' and cannot be overridden.

    Both the public ``attn_implementation`` property and the private
    ``_attn_implementation`` attribute are blocked to prevent
    ``_maybe_use_eager_attention`` from bypassing the no-op setter.
    """
    ...
    def __setattr__(self, name: str, value: object) -> None:
        if name in ("attn_implementation", "_attn_implementation"):
            return None
...
# ===========================================================================
# Proper attention models for feature correctness tests
# ===========================================================================
...
class DummyProperAttnModule(nn.Module):
    ...
    Computes standard scaled-dot-product attention: output = softmax(QK^T/√d) @ V.
    Stores per-batch attention weights on ``self.attn_weights`` so that hooks
    can retrieve them via ``_extract_attention_weights``.
    """
...
class DummyLlamaProperAttnModel(nn.Module):
    ...
    the default layer/attention field fallback logic in the hook manager.
    """
...
def test_value_vectors_yield_attention_output(monkeypatch):
    ...
    computed_output = (attn_weights @ value).transpose(0, 1).reshape(seq_len, hidden_size)
...
def test_last_hidden_state_plus_ln_final_matches_model_output(monkeypatch):
    ...
    last_layer_output = results[0].layer_features[0].output  # (seq, hidden_size), pre-ln_final
    last_hidden_state = model_output.last_hidden_state[0]  # (seq, hidden_size), post-ln_final
